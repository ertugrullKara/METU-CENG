    ...
    for j in xrange(F): # jth filter
      ...
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          # FILL IN THIS PART
          # 
          windows = padded[i, :, hs:hs+HH, ws:ws+WW]
          out[i,j,k,l] = np.sum(windows*w[j]) + b[j]
  cache = x, w, b, conv_param
    ...
    for j in xrange(C):
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          max_p = np.max(window)
          dx[i, j, hs:hs+HH, ws:ws+WW] += (window == max_p) * dout[i, j, k, l]
...
def mse_loss(x, y):
  ...
  - dx: Gradient of the loss with respect to x
  """
  loss = 0.5 * np.sum(np.square(x-y).mean(axis=1))
  return loss, x-y
