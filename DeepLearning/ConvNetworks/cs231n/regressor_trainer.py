import numpy as np
from cs231n.layers import mse_loss
...
class RegressionTrainer(object):
  ...
  def train(self, X, y, X_val, y_val, 
    ...
    epoch = 0
    best_val_acc = 9999999999999999999999999999999999999999999999
    best_model = {}
    loss_history = []
    train_acc_history = []
    val_acc_history = []
    for it in xrange(num_iters):
      ...
      if first_it or epoch_end or acc_check:
        ...
        y_pred_train = scores_train
        train_acc, _ = mse_loss(y_pred_train, y_train_subset)
        train_acc_history.append(train_acc)

        # evaluate val accuracy
        scores_val = loss_function(X_val, model)
        y_pred_val = scores_val
        val_acc, _ = mse_loss(y_pred_val, y_val)
