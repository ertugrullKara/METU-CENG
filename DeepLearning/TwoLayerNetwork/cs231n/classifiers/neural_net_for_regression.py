...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    scores -= y
    loss = 0.5 * np.sum(np.square(scores))
    ...
    #############################################################################
    # Scores value is now actually scores - y
    grads['W2'] = np.dot(h1.T, scores) + reg * W2
    grads['b2'] = np.sum(scores, axis=0)
    
    scores = np.dot(scores, W2.T)
    scores[h1 == 0] = 0
    grads['W1'] = np.dot(X.T, scores) + reg * W1
